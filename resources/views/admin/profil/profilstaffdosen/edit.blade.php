...
@section('content')
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">{{ __('Dosen FH') }}</h1>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="./">{{ __('Home') }}</a></li>
            <li class="breadcrumb-item">{{ __('Profil') }}</li>
            <li class="breadcrumb-item">{{ __('Dosen FH') }}</li>
        ...
        <div class="col-lg-12">
            <div class="card mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                    <h6 class="m-0 font-weight-bold text-primary">{{ __('Edit Dosen FH') }}</h6>
                ...
                <div class="table-responsive p-3">
                    <form action="{{ route('staff-dosen-fh.update', $dosen->id) }}" method="POST" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                                ...
                                <div class="col border-right">
                                    <div class="p-3 py-5">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            <h4 class="text-right">Dosen FH</h4>
                                        </div>
                                        <div class="row mt-2">
                                            <div class="col-md-12">
                                                ...
                                                <select class="form-control" name="title" @error('title') is-invalid @enderror placeholder="judul">
                                                    ...
                                                    <option @if($dosen->title == "HUKUM") selected @endif value="Hukum">Hukum</option>
                                                </select>
                                                @error('title')
                                                    <small class="text-danger">{{ $message }}</small>
                                                @enderror
